...
const BIRTHDAY = new Date(2006, 8, 18); // 18.09.2006 (month is 0-indexed)

const get_age = () => {
  const today = new Date();
  let age = today.getFullYear() - BIRTHDAY.getFullYear();
  const has_birthday_passed = (
    today.getMonth() > BIRTHDAY.getMonth() ||
    (today.getMonth() === BIRTHDAY.getMonth() && today.getDate() >= BIRTHDAY.getDate())
  );
  if(!has_birthday_passed) age -= 1;
  return String(age);
};
...
const About = {
  info: [
    ...
    { fieldName: "Level", fieldNameHR: "Age", fieldValue: get_age(), icon: "⭐" },
